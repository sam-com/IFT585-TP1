...
public:
	...
	uint32_t getContenu();					// Retourne la totalité du contenu

	void setType(uint32_t type);			// Affecte le type de la trame
	void setSequence(uint32_t sequence);	// Affecte la séquence
	void setDonnees(uint32_t donnees);		// Affecte les données
